...
return tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, './Fun/ping.gif', '@id_admin_MaTaDoR', dl_cb, nil)
end
--------------------------------
...
--------------------------------
	if matches[1]:lower() == "photo" or  matches[1]:lower() == "Photo" or matches[1]:lower() == "عکس" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@id_admin_MaTaDoR", dl_cb, nil)
	end
